...
interface CountryVATCalculatorProps {
  preselectedCountry?: string;
  preselectedRate?: number;
}

export default function CountryVATCalculator({ 
  preselectedCountry = 'DE', 
  preselectedRate 
}: CountryVATCalculatorProps) {
  const t = useTranslations();
  const [amount, setAmount] = useState<string>('100');
  const [country, setCountry] = useState<string>(preselectedCountry);
  const [rate, setRate] = useState<number>(preselectedRate || 19);
  ...
  useEffect(() => {
    if (preselectedCountry) {
      ...
      const countryData = getCountryByCode(preselectedCountry);
      if (countryData) {
        setRate(preselectedRate || countryData.standard_rate);
      }
    }
  }, [preselectedCountry, preselectedRate]);
  ...
  }, [amount, rate, direction, selectedCountry]);
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      {/* AdSense Placeholder */}
      <div className="bg-gray-100 border-2 border-dashed border-gray-300 rounded-lg p-4 text-center text-gray-500 mb-6">
        <p>{t('advertisement')}</p>
        <p className="text-sm">{t('bannerAd')}</p>
      </div>
    </div>
  );
}